...
export default function TelemedicineScreen() {
  ...
  const updateSession = usePatientStore((s) => s.updateSession);
      ...
      <ScrollView contentContainerStyle={styles.content}>
        {patient && (
          <Card variant="elevated" style={styles.summaryCard}>
            ...
            <Text style={styles.line}>Age: {patient.age} | Sex: {patient.sex}</Text>
            {cnn && (
              <>
                <View style={[styles.riskPill, { backgroundColor: `${riskColor}12` }]}>
                  <Text style={[styles.riskPillText, { color: riskColor }]}>
                    Pneumonia Risk: {cnn.pneumoniaRiskBucket.toUpperCase()}
                  </Text>
                </View>
                <Text style={styles.line}>
                  Confidence: {(cnn.confidence * 100).toFixed(0)}% • Signal: {cnn.signalSource}
                </Text>
              </>
            )}
          </Card>
        ...
        {sent ? (
          <Card variant="elevated" style={styles.successCard}>
            ...
            <Text style={styles.successTitle}>{t('telemedicine.referralSent')}</Text>
          ...
          <View style={styles.actions}>
            ...
            />
            <Button
              title={t('telemedicine.scheduleVisit')}
              onPress={() => router.replace('/home')}
              variant="secondary"
            />
          </View>
...
const styles = StyleSheet.create({
  ...
  summaryTitle: { fontSize: 14, fontWeight: '600', color: '#111827', marginBottom: 10 },
  line: { fontSize: 13, color: '#6B7280', marginBottom: 4 },
  riskPill: {
  ...
  refId: { fontSize: 13, color: '#6B7280', marginTop: 6 },
